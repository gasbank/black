    ...
    internal static class Program
    {
        ...
        static void ExecuteDetermineIslandTest(string sourceFileName, string bytesFileName) {
            ...
            using (var bytesFileStream = new FileStream(bytesFileName, FileMode.Open)) {
                ...
                try {
                    ...
                    stageData = JsonSerializer.Deserialize<StageData>(bytesFileStream) ?? throw new NullReferenceException("StageData null");
            ...
            using (var image = Image.Load<Rgba32>(sourceFileName)) {
                foreach (var island in stageData.islandDataByMinPoint) {
                    ...
                    var fillMinPoint = FloodFill.ExecuteFillIf(image, minPoint, White, targetColor, out var pixelArea, out _, out _);
        ...
        static string ExecuteDetermineIsland(string sourceFileName, string startFileName) {
            ...
            for (int h = 0; h < image.Height; h++) {
                for (int w = 0; w < image.Width; w++) {
                    ...
                    if (pixelColor == Black) {
                        ...
                        var fillMinPoint = FloodFill.ExecuteFillIfNotBlack(image, coord, Black, out var pixelArea, out var points, out var originalColors);
                        if (fillMinPoint != new Vector2Int(image.Width, image.Height)) {
                            if (originalColors.Count > 1) {
                                ...
                                var prominentColor = originalColors.OrderByDescending(e => e.Value).First(e => e.Key != White).Key;
            ...
            using (var stream = File.Create(outputPath)) {
                ...
                JsonSerializer.Serialize(stream, stageData);
                stream.Close();
            }
        ...
        static string ExecuteQuantize(string sourceFileName, int maxColor = 30) {
            ...
            var quantizer = new WuQuantizer(new QuantizerOptions{MaxColors = maxColor});
            ...
            q.BuildPalette(new DefaultPixelSamplingStrategy(), image.Frames[0]);
            ...
            var encoder = new SixLabors.ImageSharp.Formats.Png.PngEncoder {
                Quantizer = new PaletteQuantizer(quantizedPalette.ToArray(), new QuantizerOptions()),
                ...
                for (int h = 0; h < image.Height; h++) {
                    for (int w = 0; w < image.Width; w++) {
                        ...
                        if (pixelColor == Black) {
                            ...
                            // 수집한 모든 픽셀은 points에, points의 min point는 반환값으로 받는다.
                            var coord = new Vector2Int(w, h);
                            var fillMinPoint = FloodFill.ExecuteFillIfNotBlack(image, coord, Black, out var pixelArea, out _, out _);
                            if (fillMinPoint != new Vector2Int(image.Width, image.Height)) {
                                islandPixelAreaByMinPoint[fillMinPoint] = pixelArea;
                            } else {
                                throw new Exception("Invalid fill min point!");
            ...
            using (var image = Image.Load<Rgba32>(sourceFileName)) {
                foreach (var island in islandPixelAreaByMinPoint) {
                    if (island.Value < threshold) {
                        var fillMinPoint = FloodFill.ExecuteFillIfNotBlack(image, island.Key, Black, out var pixelArea, out _, out _);
