...
public class PlatformLogin : MonoBehaviour {
    ...
    [SerializeField] Text userId = null;
    [SerializeField] CanvasGroup rootCanvasGroup = null;
    ...
    void StartLogin() {
        ...
        if (platform.DisableLoginOnStart) {
            PlatformInterface.instance.logger.Log("PlatformLogin: Skipping StartLogin() since DisableLoginOnStart is true.");
            return;
        }
            ...
            platform.StartAuthAsync((result, reason) => {
                ...
                if (result) {
                    if (Social.localUser != null) {
                        PlatformInterface.instance.logger.LogFormat("PlatformLogin: Social.localUser userName={0}, userId={1}", Social.localUser.userName, Social.localUser.id);
                        if (userName) {
    ...
    void OnApplicationPause(bool pause) {
        ...
        if (pause) {
            if (atLeastTriedOnce && IsAuthenticated == false) {
                PlatformInterface.instance.logger.Log("PlatformLogin: Not authenticated even if we tried. User probably logged out from service.");
