...
public class BlackCustomFormatter : IFormatProvider, ICustomFormatter
{
    ...
    {
        if (Equals(formatProvider) == false) return null;

        if (format == "msec3")
        {
            if (arg is IFormattable) return (long.Parse(arg.ToString()) / 1000.0f).ToString("F3");
        }
        ...
        else if (format == "plural_yies")
        {
            ...
            // 1 energ[y  ]
            // 2 energ[ies]
            if (arg is IFormattable || arg is string) return arg.ToString() == "1" ? "y" : "ies";
        }
        else if (format == "plural_isare")
        {
            // 0 gems are needed
            // 1 gem is needed
            if (arg is IFormattable || arg is string) return arg.ToString() == "1" ? "is" : "are";
        }
